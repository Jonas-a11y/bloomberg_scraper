...
from app.database import DB_PATH, NETWORK_DB_PATH, get_db
...
def _attachment(content, filename):
    return Response(
        content=content,
...
def _csv_response(rows, filename):
    ...
    return _attachment(output.getvalue().encode("utf-8"), filename)


def _json_response(rows, filename):
    return _attachment(json.dumps(rows, indent=2).encode("utf-8"), filename)
...
@router.get("/export/network.db")
def export_network_db():
    return FileResponse(
        path=str(NETWORK_DB_PATH),
        media_type="application/octet-stream",
        filename="network.db",
    )
